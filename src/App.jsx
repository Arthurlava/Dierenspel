...
export default function App() {
        ...
        onValue(r, (snap) => {
            ...
            if (!data.hostId || !data.players[data.hostId] || !data.turn || !data.players[data.turn]) {
                runTransaction(ref(db, `rooms_animals/${code}`), (d) => {
                    if (!d || !d.players) return d; const ids = Object.keys(d.players);
    ...
    useEffect(() => {
        if (!roomCode || !room) return; if (room.solo) return; if (room.paused) return;
    ...
    async function resumeGame() {
        ...
        await runTransaction(ref(db, `rooms_animals/${roomCode}`), (d) => {
            if (!d) return d; if (!d.paused) return d;
    ...
    async function submitAnimal() {
        ...
        await runTransaction(r, (d) => {
            if (!d) return d; if (d.paused) return d; // niet verwerken tijdens pauze
    ...
    async function useJilla() {
        ...
        await runTransaction(r, (d) => {
            ...
            if (isMP) {
                d.scores ??= {}; d.stats ??= {}; d.scores[playerId] = (d.scores[playerId] || 0) - JILLA_PENALTY;
