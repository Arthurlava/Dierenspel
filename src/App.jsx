...
/* --------- Firebase (hardcoded; later evt. ENV) --------- */
...
/* --------- Stijl (terug naar donker + groen, single column) --------- */
const GlobalStyle = () => (
    <style>{`
    :root{
      --bg1:#171717; --bg2:#262626; --panel:rgba(255,255,255,.06);
      --border:rgba(255,255,255,.14); --text:#fff; --muted:rgba(255,255,255,.7);
      --brand1:#22c55e; --brand2:#16a34a; --warn:#dc2626; --alt:#065f46;
    }
    *{box-sizing:border-box}
    ...
    .btn.alt{background:var(--alt);color:#eafff6}
    ...
    .letterBig{display:grid;place-items:center;width:120px;height:120px;border-radius:28px;
      background:radial-gradient(circle at 30% 30%, var(--brand1), var(--brand2));
      color:#041507;font-size:56px;font-weight:900;border:1px solid rgba(0,0,0,.2);
      box-shadow:0 14px 40px rgba(34,197,94,.35); margin:8px auto 12px;
    }
...
export default function App() {
    ...
    async function createRoom() {
        ...
        const obj = {
            ...
            jail: {}, scores: {}, stats: {},
            phase: "answer", turnStartAt: null, cooldownEndAt: null, version: 3
        };
    ...
    async function checkAnimalViaAPI() {
        ...
        try {
            ...
            if (json.found) {
                const name = json.vernacularName || json.scientificName || q;
                setApiState({ status: "ok", msg: `✅ Gevonden: ${name} (${json.source}, conf. ${json.confidence ?? 0})` });
            } else {
                setApiState({ status: "notfound", msg: "ℹ️ Niet gevonden in database" });
            }
        } catch {
            setApiState({ status: "error", msg: "Netwerkfout" });
        }
    }
